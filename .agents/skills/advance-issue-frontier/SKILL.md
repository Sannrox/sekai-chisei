...
surface: `proto/`, SQLite versus PostgreSQL migrations, gateway versus native
gRPC, or the same crate or ownership boundary. Read each candidate's affected
boundary and scope; when two candidates name the same surface, present them as
sequential, not parallel, and report the collision for maintainer judgment.
