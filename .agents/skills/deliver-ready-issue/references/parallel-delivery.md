...
several machines. The project operating system
(`docs/project-operating-system.md`, "Parallel delivery lanes") owns the
policy: lane definition, GitHub claims, the three-lane limit, collision
surfaces, isolation, and serialization. This reference owns the lead procedure.
...
3. Reject candidate pairs that touch the same collision surface: `proto/`,
   SQLite versus PostgreSQL migrations, gateway versus native gRPC, or the
   same crate or ownership boundary. Run such Issues in sequence.
