...
1. Stage only the intended paths and create a narrow imperative commit.
   ...
   shared surface (`proto/`, SQLite versus PostgreSQL migrations, gateway
   versus native gRPC, the same crate). Do not rebase merely because `main`
   advanced.
...
3. Publish to the claim branch with **GitHub-verified** commits via
...
5. Open the Pull Request as a draft with the first published commit, then mark
   ...
   - may list public lane fields only: claim branch, repo-relative worktree
     (for example `.worktrees/issue-N`), base SHA, published SHA, authority
     ceiling. Never hostname, FQDN, home path, absolute worktree path, LAN
     or employer network name, or other environment inventory;
