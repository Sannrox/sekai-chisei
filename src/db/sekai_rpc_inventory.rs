...
mod tests {
    ...
    fn inventory_matches_proto_and_evidence_paths() {
        ...
        for rpc in [
            ...
            "PutClassificationLattice",
            "GetClassificationLattice",
            "SimulateObjectPolicyChange",
            "QueryObjectPolicyAudit",
        ] {
            assert!(inventory.entry(rpc).is_some(), "missing {rpc}");
        }
        ...
        assert!(inventory.entry("CloseDefinitionProposal").is_some());
        assert!(inventory.entry("GetPublishedDefinitionRevision").is_some());
        assert!(inventory.entry("CompareDefinitionRevisions").is_some());
        assert!(inventory.entry("ReportDefinitionConsumerImpact").is_some());
        assert!(
            inventory
                .entry("ClassifyDefinitionRevisionCompatibility")
                .is_some()
        );
        assert!(inventory.entry("ExecuteDefinitionFactMigration").is_some());
        assert!(inventory.entry("GetDefinitionFactMigration").is_some());
        ...
        assert!(inventory.entry("EvaluateObjectSet").is_some());
        assert!(inventory.entry("ReadObjectChangeSubscription").is_some());
        assert!(inventory.entry("DescribeObjectAction").is_some());
        assert!(inventory.entry("PreviewObjectAction").is_some());
        assert!(inventory.entry("GetActionInstance").is_some());
        assert!(inventory.entry("ListActionInstances").is_some());
