...
impl SekaiDb {
    ...
    pub fn validate_execution_evidence_envelope(
        ...
        if envelope.source_record_id != report.redemption_id {
            return Err("execution evidence source record does not match its redemption".into());
        }
                    ...
                    "SELECT r.redemption_json,r.evidence_due_at_ms
                     ...
                     LEFT JOIN sekai_external_action_execution_state s
                       ON s.redemption_id=r.redemption_id AND s.terminal=1
                          ...
                          AND EXISTS(
                            SELECT 1 FROM sekai_external_action_execution_evidence e
                            WHERE e.redemption_id=s.redemption_id
                              AND e.source_sequence=s.source_sequence
                              AND e.projection_status='projected'
                              AND CAST(json_extract(e.evidence_json,'$.finished_at_ms') AS INTEGER)
                                  <=r.evidence_due_at_ms
                          )
                ...
                .query_row(
                    "SELECT NOT EXISTS(
                         ...
                         WHERE redemption_id=?1 AND terminal=1 AND updated_at_ms<=?3
                           AND EXISTS(
                             SELECT 1 FROM sekai_external_action_execution_evidence e
                             WHERE e.redemption_id=?1
                               AND e.source_sequence=sekai_external_action_execution_state.source_sequence
                               AND e.projection_status='projected'
                               AND CAST(json_extract(e.evidence_json,'$.finished_at_ms') AS INTEGER)<=?3
                           )
                     ) AND NOT EXISTS(
...
mod tests {
    ...
    fn envelope(report: &ExecutionEvidence, sequence: i64) -> EvidenceEnvelope {
        ...
        EvidenceEnvelope {
            ...
            source_record_id: report.redemption_id.clone(),
            ...
            idempotency_key: format!("redemption-{}-{sequence}", report.redemption_id),
    ...
    fn admitted_funnel_reports_advance_without_collapsing_terminal_state() {
        ...
        let mut invalid = report(ExecutionLifecycleState::Accepted);
        invalid.host_identity = "executor:other".into();
        let mut invalid = envelope(&invalid, 10);
        invalid.producer_identity = "executor:file".into();
        let rejected = db
            .submit_evidence(&invalid, "executor:file", invalid.observed_at_ms)
            .unwrap();
        assert!(rejected.accepted);
        db.reject_evidence_submission(
            &rejected.submission.id,
            invalid.observed_at_ms,
            "invalid_execution_evidence",
            "host identity does not match authenticated producer",
        )
        .unwrap();

        let corrected = envelope(&report(ExecutionLifecycleState::Accepted), 10);
        let corrected = db
            .submit_evidence(&corrected, "executor:file", corrected.observed_at_ms)
            .unwrap();
        assert!(corrected.accepted);
        assert!(!corrected.deduplicated);
        ...
        let terminal_at_ms: i64 = db
            .conn()
            .query_row(
                "SELECT updated_at_ms FROM sekai_external_action_execution_state
                 WHERE redemption_id='redemption-1'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        assert_eq!(terminal_at_ms, 1_503);
        assert!(
        ...
        let reused_execution = envelope(&reused_execution, 1);
        let admitted = db
            .submit_evidence(
                &reused_execution,
                "executor:file",
                reused_execution.observed_at_ms,
            )
            .unwrap();
        assert!(admitted.accepted);
