...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match args[0].as_str() {
        ...
        "workflow" => {
            sekai_chisei::workflow_action_cli::run_workflow_command(
                args.into_iter().skip(1).collect(),
            )
            .await
        }
        ...
        "warehouse" => {
            sekai_chisei::warehouse_projection_cli::run_warehouse_command(
                args.into_iter().skip(1).collect(),
            )
            .await
        }
        "lakehouse" => {
            sekai_chisei::lakehouse_snapshot_cli::run_lakehouse_command(
                args.into_iter().skip(1).collect(),
            )
            .await
        }
        "providers" => {
            sekai_chisei::model_platform_cli::run_providers_command(
                args.into_iter().skip(1).collect(),
            )
            .await
        }
        ...
        "sdk-packages" => {
            sekai_chisei::client_package_cli::run_sdk_packages_command(
                args.into_iter().skip(1).collect(),
            )
            .await
        }
