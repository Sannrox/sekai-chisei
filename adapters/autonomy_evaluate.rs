//! Domain-neutral evaluation autonomy adapter (#715).

use sekai_chisei::sekai::autonomous_envelope::{
    AutonomousEnvelope, AutonomousPins, ENVELOPE_CONTRACT, PROFILE_EVALUATE, PROFILE_VERSION,
};
use serde::Deserialize;

pub const ADAPTER_ID: &str = PROFILE_EVALUATE;
...
pub fn parse(bytes: &[u8]) -> Result<EvaluateDocument, String> {
    serde_json::from_slice(bytes).map_err(|error| format!("evaluate document is invalid: {error}"))
}

pub fn translate_envelope(document: &EvaluateDocument) -> Result<AutonomousEnvelope, String> {
    Ok(AutonomousEnvelope {
        ...
        adapter_id: PROFILE_EVALUATE.into(),
