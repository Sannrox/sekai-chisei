//! Domain-neutral inventory warehouse adapter (#711).

use sekai_chisei::sekai::warehouse_projection::{
    PROFILE_INVENTORY, PROFILE_VERSION, SECURITY_CONTRACT, WAREHOUSE_CONTRACT, WarehouseColumn,
    WarehouseCursor, WarehouseProjection, WarehouseRow, WarehouseSecurity,
};
use serde::Deserialize;
use std::collections::BTreeMap;

pub const ADAPTER_ID: &str = PROFILE_INVENTORY;
...
pub fn parse(bytes: &[u8]) -> Result<InventoryDocument, String> {
    serde_json::from_slice(bytes).map_err(|error| format!("inventory document is invalid: {error}"))
}

pub fn translate_projection(document: &InventoryDocument) -> Result<WarehouseProjection, String> {
    Ok(WarehouseProjection {
        ...
        adapter_id: PROFILE_INVENTORY.into(),
...
pub fn translate_rows(document: &InventoryDocument) -> Result<Vec<WarehouseRow>, String> {
    document
        .rows
        .iter()
        .map(|row| {
            if row.id.trim().is_empty() || row.sku.trim().is_empty() {
                return Err("inventory row identity is required".into());
            }
            Ok(WarehouseRow {
                row_id: row.id.clone(),
