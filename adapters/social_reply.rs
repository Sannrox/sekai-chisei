...
pub fn translate(document: ReplyDocument) -> Result<EvidenceDraft, String> {
    let reply_id = sdk::require_nonempty(&document.reply_id, "reply_id")?;
    let parent_post_id = sdk::require_nonempty(&document.parent_post_id, "parent_post_id")?;
    let author_reference = sdk::require_nonempty(&document.author_reference, "author_reference")?;
    ...
    let observed_at_ms = sdk::parse_rfc3339_millis(&document.created_at, "created_at")?;
    let collected_at_ms = match document.collected_at.as_deref() {
        Some(value) => sdk::parse_rfc3339_millis(value, "collected_at")?,
        None => observed_at_ms,
    };
