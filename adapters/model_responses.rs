//! Domain-neutral Responses model-platform adapter (#713).
...
use sekai_chisei::sekai::model_platform::{
    ...
    ModelUsageFixture, PROFILE_RESPONSES, PROFILE_VERSION,
};
use serde::Deserialize;

pub const ADAPTER_ID: &str = PROFILE_RESPONSES;
...
pub fn parse(bytes: &[u8]) -> Result<ResponsesDocument, String> {
    serde_json::from_slice(bytes).map_err(|error| format!("responses document is invalid: {error}"))
}
...
pub fn translate_certification(
    ...
    Ok(ModelPlatformCertification {
        ...
        adapter_id: PROFILE_RESPONSES.into(),
