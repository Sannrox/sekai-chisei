//! Domain-neutral metrics lakehouse adapter (#712).
...
use sekai_chisei::sekai::lakehouse_snapshot::{
    ...
    LakehouseSnapshot, PROFILE_METRICS, PROFILE_VERSION, SECURITY_CONTRACT,
};
use serde::Deserialize;
use std::collections::BTreeMap;

pub const ADAPTER_ID: &str = PROFILE_METRICS;
...
pub struct MetricsDocument {
    ...
    pub partitions: Vec<MetricsPartition>,
}
...
pub fn parse(bytes: &[u8]) -> Result<MetricsDocument, String> {
    serde_json::from_slice(bytes).map_err(|error| format!("metrics document is invalid: {error}"))
}

pub fn translate_snapshot(document: &MetricsDocument) -> Result<LakehouseSnapshot, String> {
    Ok(LakehouseSnapshot {
        ...
        adapter_id: PROFILE_METRICS.into(),
            ...
            .map(|partition| {
                Ok(LakehousePartition {
                    ...
                    rows: partition
                        ...
                        .iter()
                        .map(|row| {
                            if row.id.trim().is_empty() || row.value.trim().is_empty() {
                                return Err("metrics row identity is required".into());
                            }
                            Ok(LakehouseRow {
                                row_id: row.id.clone(),
