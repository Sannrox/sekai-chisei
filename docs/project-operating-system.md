...
`proto/`, SQLite versus PostgreSQL migrations, gateway versus native gRPC,
and the same crate or ownership boundary. Lanes that would both change one of
...
4. For a delivery lane, claim `<type>/<issue>` on GitHub and check it out in
   `.worktrees/issue-<issue>`. Human topic branches may use
   `<type>/<issue>-<slug>`.
...
Delivery lanes use `<type>/<issue>` without a slug, for example `fix/142` or
`feat/203`, because the branch name is the cross-machine claim (see "Parallel
delivery lanes"). Human topic branches may add a slug, for example
`fix/142-budget-reconciliation`. An agent delivering an Issue checks the claim
branch out in the worktree `.worktrees/issue-<issue>`, never in the primary
checkout. Existing `codex/<issue>-<slug>` branches are legacy lanes; they still
count as claims until they land or their owner removes them.
