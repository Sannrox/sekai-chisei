...
| `SEKAI_OBJECT_INDEX_ENGINE` | `hop-projection` | Object-set hop engine (`hop-projection` default, or `nested-loop` as an explicit debug scan). Both engines fail closed while hop-projection ready is false after a binding-generation bump. Switching engines requires `ReindexObjectType` |
