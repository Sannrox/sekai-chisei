...
| `SEKAI_OBJECT_LOG_HOST` | unset | `host:port` of a mikura object-log host (ADR 0088). Admits append and read generations through it instead of a local log. Mutually exclusive with `SEKAI_OBJECT_LOG`; setting both fails every ingest closed |
